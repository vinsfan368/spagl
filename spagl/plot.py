...
def add_log_scale_imshow_alt(axes, diff_coefs, fontsize=None, side="x"):
    """
...
def rbme_likelihood_plot(tracks, diff_coefs=None, loc_errors=None,
    ...
    if log_x_axis:
        add_log_scale_imshow_alt(axes, diff_coefs, fontsize=fontsize, side="x")
    else:
        n_labels = 7
        nD = len(diff_coefs)
        m = nD // n_labels 
    ...
    # Make the y-axis for the localization error
    n_labels = 7
    nLE = len(loc_errors)
